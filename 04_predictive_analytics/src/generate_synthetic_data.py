...
def generate_clean_data(seed=SEED):
    ...
    for cohort_idx, scoring_date in enumerate(SCORING_DATES):
        ...
        txn60_lambda = np.clip(np.exp(1.55 + 0.38 * engagement + 0.20 * latent_value - 0.10 * drift), 0.2, 30)
        tx60 = rng.poisson(txn60_lambda)
        tx60 = np.maximum(tx60, 1)  # eligibility requires at least one prior-60d transaction

        recent_share = np.clip(0.48 + 0.10 * engagement - 0.05 * drift + rng.normal(0, 0.08, N_CUSTOMERS), 0.08, 0.85)
        ...
        active_days = np.minimum(30, np.maximum(1, tx30 - rng.poisson(np.maximum(tx30 * 0.18, 0.2))))
        login_days = np.clip((4 + 2.2 * engagement + 0.45 * active_days + rng.normal(0, 3, N_CUSTOMERS)).round(), 0, 30).astype(int)
        products_used = np.clip((1.8 + 0.45 * engagement + 0.35 * latent_value + rng.normal(0, 0.7, N_CUSTOMERS)).round(), 1, 6).astype(int)

        avg_value = np.clip(rng.lognormal(mean=np.log(55 + 14 * np.maximum(latent_value, -1.5)), sigma=0.55), 5, 1200)
        payment_volume = np.clip(avg_value * np.maximum(tx30, 1) * rng.lognormal(0, 0.18, N_CUSTOMERS), 0, 25_000)

        # Behavioral decline relative to prior 30 days.
        prior30 = np.maximum(1, (tx30 / np.clip(0.95 + 0.12 * engagement + rng.normal(0, 0.18, N_CUSTOMERS), 0.35, 1.7)).round().astype(int))
        count_change = (tx30 - prior30) / prior30
        prior_volume = np.maximum(10, payment_volume / np.clip(0.96 + 0.10 * engagement + rng.normal(0, 0.20, N_CUSTOMERS), 0.35, 1.8))
        volume_change = (payment_volume - prior_volume) / prior_volume

        failed = rng.poisson(np.clip(0.35 + 0.08 * tx30 + np.where(country == "BR", 0.15, 0), 0.05, 6))
        support = rng.poisson(np.clip(0.22 + 0.09 * failed + np.where(engagement < -0.8, 0.20, 0), 0.02, 4))
        marketing = np.clip(_sigmoid(0.9 * engagement + 0.18 * login_days - 0.10 * days_since_last + rng.normal(0, 0.7, N_CUSTOMERS)), 0, 1)

        value_score = 0.55 * latent_value + 0.03 * tx30 + 0.0015 * payment_volume
        value_tier = pd.cut(value_score, bins=[-np.inf, 0.0, 1.2, np.inf], labels=["Low", "Medium", "High"]).astype(str)

        # Target risk: strong recency + decline effects, engagement protection, segment differences, and drift.
        logit = -2.15
        ...
        future_txn_lambda = np.where(target == 1, 0.0, np.clip(2.0 + 0.22 * tx30 + 0.6 * engagement, 0.3, 15))
        future_txn = np.where(target == 1, 0, np.maximum(1, rng.poisson(future_txn_lambda)))
        future_status = np.where(target == 1, "Inactive", "Active")
        retention_case = rng.binomial(1, np.clip(0.08 + 0.66 * target + 0.10 * (days_since_last > 20), 0, 0.95))
...
def build_training_sample(raw, n=2500, seed=SEED):
    ...
    base = raw.drop_duplicates(["customer_id", "scoring_date"]).sample(n=n - 30, random_state=seed).copy()
